...
#args <- list("rjfullmodel_pruned", 10000, "_r1")

td <- readRDS("../output/cleandata/matchednewdata.rds")
td <- filter(td, !is.na(support), !is.na(precip))
tree <- td$phy
dat <- td$dat
...
custom.lik <- function(pars, cache, X, model="Custom"){
  ...
  X = X - pars$beta1[betaID]*pred$lnVn - pars$betaC*pred$growthC - pars$beta1C*pred$growthC*pred$lnVn - pars$betaTemp*pred$tempK - pars$betaPrecip*pred$precip
  cache$dat <- X
...
startpar <- list(alpha=0.1, sig2=3, beta1=rnorm(k+1, -1, 1),betaC=0, beta1C=0, betaTemp=0, betaPrecip=0,k=k, ntheta=k+1, theta=rnorm(k+1, -1.25, 0.25), sb=sb, loc=rep(0, k), t2=2:(k+1))

monitor = function(i, lik, pr, pars, accept, accept.type, j){
  names <- c("gen", "lnL", "prior", "alpha","sig2", "rbeta1","betaC","beta1C", "betaTemp","betaPrecip","rtheta", "k")
  ...
  cat(sprintf(string, i, lik, pr, pars$alpha, pars$sig2, pars$beta1[1], pars$theta[1], pars$betaC,pars$beta1C, pars$betaTemp, pars$betaPrecip, pars$k), sprintf("%-8.2f", acceptratios),"\n", sep="")
}
...
model.WA <- list(moves = list(alpha=".multiplierProposal", sig2=".multiplierProposal", 
                                   beta1=".vectorMultiplier",betaC=".slidingWindowProposal",beta1C=".slidingWindowProposal", betaTemp=".slidingWindowProposal",
                              betaPrecip=".slidingWindowProposal",
                              theta=".adjustTheta", k=".splitmergebd", slide=".slide"
          ),
control.weights = list(alpha=4, sig2=2, beta1=10, betaC=4,  beta1C=4,betaTemp=4, betaPrecip=4,
                       k=6, theta=10, slide=2 
          ),
D = list(alpha=0.75, sig2= 0.5, beta1=0.75,betaC=0.4, beta1C=0.2, betaTemp=0.001,betaPrecip=0.001, k=c(1,1), theta=2, slide=1
          ),
...
prior <- make.prior(tree, plot.prior = FALSE, 
                    dists=list(dalpha="dhalfcauchy", dsig2="dhalfcauchy", dbeta1="dnorm",
                               dbetaC="dnorm", dbeta1C="dnorm", dbetaTemp="dnorm", dbetaPrecip="dnorm",
                               dsb="dsb", dk="cdpois", dtheta="dnorm"
                    ), 
                    param=list(dalpha=list(scale=1), dsig2=list(scale=1), dbeta1=list(mean=-2, sd=1),
                               dbetaC=list(mean=0, sd=1), dbeta1C=list(mean=0, sd=0.5), dbetaTemp=list(mean=0, sd=0.002),dbetaPrecip=list(mean=0, sd=0.005),
...
saveRDS(chain, file=paste("../output/runs/",args[[1]], "/", args[[1]], args[[3]],"_chain.rds",sep=""))
saveRDS(mymcmc, file=paste("../output/runs/",args[[1]], "/", args[[1]], args[[3]],"_mcmc.rds",sep=""))
