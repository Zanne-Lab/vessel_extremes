...
OU4.fits <- lapply(models, function(x) lapply(smtrees$OU4[1:2], function(y) OUwie(y, OUwie.OU4, model = x, simmap.tree=TRUE)) )

OUwie.OU2ph <- data.frame('Genus_species'=tdOUwie$phy$tip.label, Reg=tdOUwie$dat$OU2ph, X=tdOUwie$dat$lnVs)
OU2ph.fits <- lapply(models, function(x) lapply(smtrees$OU2ph[1:2], function(y) OUwie(y, OUwie.OU2ph, model = x, simmap.tree=TRUE)) )

OUwie.OU2fr <- data.frame('Genus_species'=tdOUwie$phy$tip.label, Reg=tdOUwie$dat$OU2fr, X=tdOUwie$dat$lnVs)
OU2fr.fits <- lapply(models, function(x) lapply(smtrees$OU2fr[1:2], function(y) OUwie(y, OUwie.OU2fr, model = x, simmap.tree=TRUE)) )


ou4aics <- lapply(OU4.fits, function(x) sapply(x, function(y) y$AIC))
names(ou4aics) <- models
ou4aics <- do.call(cbind, ou4aics)
boxplot(ou4aics)
...
ou4parests <- extractModel(5, OU4.fits)
