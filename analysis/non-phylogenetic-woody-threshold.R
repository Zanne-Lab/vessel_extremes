...
data <- data[,c("gs","tmin.025","pmin.025","tseas.975","pseas.975","gbif.records","decimallatitude.025","decimallatitude.975","vesselSize","phenology","gbif.records")]
data$tmin.025 <- data$tmin.025/10
data$phenology <- as.numeric(data$phenology=="D")
data <- na.omit(data)

# Subset to top 50% of species
# - is this a bit harsh in this case? Because a lot of them have a lot of data...
data <- data[data$gbif.records >= median(data$gbif.records),]
...
sink("../output/nonphylo_models/conduit_unscaled_top50.txt")
summary(conduit.model.avg)
sink(NULL)
...
sink("../output/nonphylo_models/conduit_scaled_top50.txt")
summary(s.conduit.model.avg)
sink(NULL)
#...all good.
...
colours[data$phenology==TRUE & data$tmin.025>0] <- "red"

setEPS()
postscript("../output/figures/figure_2_temp_top50.eps")
...
text(-35, -10, "(c)", font=2, cex=2)
dev.off()
setEPS()
postscript("../output/figures/figure_2_precip_top50.eps")
par(mar=c(5.1,4.6,4.1,2.1), cex.lab=1.25, cex.axis=1.25, cex=1.25)
...
text(1.1, -10, "(d)", font=2, cex=2)
dev.off()
setEPS()
postscript("../output/figures/figure_2_temp_seas_top50.eps")
par(mar=c(5.1,4.6,4.1,2.1), cex.lab=1.25, cex.axis=1.25, cex=1.25)
...
text(1000, -10, "(e)", font=2, cex=2)
dev.off()
setEPS()
postscript("../output/figures/figure_2_precip_seas_top50.eps")
par(mar=c(5.1,4.6,4.1,2.1), cex.lab=1.25, cex.axis=1.25, cex=1.25)
...
text(30, -10, "(f)", font=2, cex=2)
dev.off()
setEPS()
postscript("../output/figures/figure_2_lat_lower_top50.eps")
par(mar=c(5.1,4.6,4.1,2.1), cex.lab=1.25, cex.axis=1.25, cex=1.25)
...
legend("topleft", legend=c("Deciduous/Freezing", "Deciduous/Non-freezing"), pch=20, col=c("darkorange","red","skyblue2","forestgreen"), bty="n")
...
text(40, -10, "(a)", font=2, cex=2)
dev.off()
setEPS()
postscript("../output/figures/figure_2_lat_upper_top50.eps")
par(mar=c(5.1,4.6,4.1,2.1), cex.lab=1.25, cex.axis=1.25, cex=1.25)
...
#...I think the seasonality effect passes the sense check - it looks like a real thin1 to me...
