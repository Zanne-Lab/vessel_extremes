...
args <- list("rjgrowthforms", 10000, "_VsVn-1_r1")
...
cache <- bayou:::.prepare.ou.univariate(tree, setNames(log(dat$vesselSize/dat$vesselNumber), tree$tip.label), pred = pred)
...
custom.lik <- function(pars, cache, X, model="Custom"){
  ...
  X = X -pars$beta1[betaID]*pred$tcoldq.me-pars$beta2[betaID]*pred$pdryq.me- pars$betaC*pred$growthC - pars$betaH*pred$growthH - pars$betaT*pred$growthT
  cache$dat <- X
...
tmp <- lm(cache$dat ~ cache$pred$tcoldq.me + cache$pred$pdryq.me)

k <- rpois(1, lambda=10)
sb <- sample(1:length(cache$bdesc), k, replace=FALSE, prob = sapply(cache$bdesc, length))
startpar <- list(alpha=0.1, sig2=3, beta1=rnorm(k+1, tmp$coef[2],0.2),beta2=rnorm(k+1, tmp$coef[3], 0.2),betaC=0, betaH=0, betaT=0,k=k, ntheta=k+1, theta=rnorm(k+1, tmp$coef[1], 0.25), sb=sb, loc=rep(0, k), t2=2:(k+1))

monitor = function(i, lik, pr, pars, accept, accept.type, j){
  names <- c("gen", "lnL", "prior", "alpha","sig2", "rbeta1","rbeta2", "betaC", "betaH","betaT","rtheta", "k")
  string <- "%-8i%-8.2f%-8.2f%-8.2f%-8.2f%-8.2f%-8.2f%-8.2f%-8.2f%-8.2f%-8.2f%-8i"
  acceptratios <- tapply(accept, accept.type, mean)
  names <- c(names, names(acceptratios))
  if(j==0){
    cat(sprintf("%-7.7s", names), "\n", sep=" ")                           
  }
  cat(sprintf(string, i, lik, pr, pars$alpha, pars$sig2, pars$beta1[1], pars$beta2[1], pars$theta[1], pars$betaC, pars$betaH, pars$betaT, pars$k), sprintf("%-8.2f", acceptratios),"\n", sep="")
}
...
model.WA <- list(moves = list(alpha=".multiplierProposal", sig2=".multiplierProposal", 
                                   beta1=".vectorMultiplier",  beta2=".vectorMultiplier", betaC=".slidingWindowProposal", betaH=".slidingWindowProposal",
                              betaT=".slidingWindowProposal",
                              theta=".adjustTheta", k=".splitmergebd", slide=".slide"
          ),
control.weights = list(alpha=4, sig2=2, beta1=10, beta2=10, betaC=4, betaH=2, betaT=2,
                       k=10, theta=10, slide=2 
          ),
D = list(alpha=1, sig2= 0.75, beta1=0.75,beta2=0.75,betaC=0.4, betaH=0.4,betaT=0.4, k=c(1,1,1), theta=2, slide=1
          ),
...
prior <- make.prior(tree, plot.prior = FALSE, 
                    ...
                    param=list(dalpha=list(scale=1), dsig2=list(scale=1), dbeta1=list(mean=0, sd=1),dbeta2=list(mean=0, sd=1),
                               dbetaC=list(mean=0, sd=1),dbetaH=list(mean=0, sd=1),dbetaT=list(mean=0, sd=1),
                               dk=list(lambda=15, kmax=50), dsb=list(prob=1, bmax=1), 
                               dtheta=list(mean=-10, sd=5)
                    )
)
...
sumpars$beta2 <- sumstats$statistics['root.beta2',1]
...
par(mfrow=c(2,2))
lev <- levels(cache$pred$growthForm)
require(wesanderson)
pal <- wes_palette("FantasticFox")
plot(cache$pred$tcoldq.me, cache$dat, pch=21, bg=pal[1:5], col=pal[1:5], main="Regressions", ylab="log Size")
curve(sumpars$theta+sumpars$beta1*x, add=TRUE, lty=2, lwd=2, col=pal[3])
curve(sumpars$theta+sumpars$betaH+sumpars$beta1*x, add=TRUE, lty=2, lwd=2, col=pal[2])
curve(sumpars$theta+sumpars$betaT+sumpars$beta1*x, add=TRUE, lty=2, lwd=2, col=pal[5])
curve(sumpars$theta+sumpars$betaC+sumpars$beta1*x, add=TRUE, lty=2, lwd=2, col=pal[1])
legend(-20, -4, legend=lev, pch=21, col=pal[1:5], pt.bg=pal[1:5])
